...
// skip EX1: don't know how to use PhotoShop
#ifndef EX2
//#define EX2
/* 
	NOTE:
	The displacement map should be put in the Material/MaterialData classes, although this would touch the 
	d3dUtil.h in Common, which is something that I try to avoid. So for EX2, I created another descriptor
	table for displacement maps, and store the index in RenderItem.
*/
#endif
...
private:
	...
	UINT mDisplacementMapHeapIndex = 0;
...
#ifdef EX2
	CD3DX12_GPU_DESCRIPTOR_HANDLE displacementMapTexDescriptor(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	displacementMapTexDescriptor.Offset(mDisplacementMapHeapIndex, mCbvSrvDescriptorSize);
	mCommandList->SetGraphicsRootDescriptorTable(5, displacementMapTexDescriptor);
#endif//EX2
...
void NormalMapApp::LoadTextures()
{
	...
	std::vector<std::string> texNames = 
	{
		"bricksDiffuseMap",			// 0
		"bricksNormalMap",			// 1
		"bricksDisplacementMap",	// 2
		"tileDiffuseMap",			// 3
		"tileNormalMap",			// 4
		"tileDisplacementMap",		// 5
		"defaultDiffuseMap",		// 6
		"defaultNormalMap",			// 7
		"skyCubeMap"				// 8
	};
	...
	std::vector<std::wstring> texFilenames = 
	{
		...
		L"../Textures/bricks2_dism.dds",				// 2
		L"../Textures/tile.dds",						// 3
		L"../Textures/tile_nmap.dds",					// 4
		L"../Textures/tile_displacementmap.dds",		// 5
		L"../Textures/white1x1.dds",					// 6
		L"../Textures/default_nmap.dds",				// 7
		L"../Textures/snowcube1024.dds"					// 8
	};
#else
	std::vector<std::string> texNames = 
	{
...
{
	CD3DX12_DESCRIPTOR_RANGE texTable0; // sky cube
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 0);

	CD3DX12_DESCRIPTOR_RANGE texTable1; // diffuse maps
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 1, 0);

#ifdef EX2
	CD3DX12_DESCRIPTOR_RANGE texTable2; // displacement map
	texTable2.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 5, 0, 2);
...
#ifdef EX2
	std::vector<ComPtr<ID3D12Resource>> displacement2DList = 
	{
		mTextures["bricksDisplacementMap"]->Resource,
		mTextures["tileDisplacementMap"]->Resource
	};
#endif //EX2
...
#ifdef EX2
	for(UINT i = 0; i < (UINT)displacement2DList.size(); ++i)
	{
		srvDesc.Format = displacement2DList[i]->GetDesc().Format;
		srvDesc.Texture2D.MipLevels = displacement2DList[i]->GetDesc().MipLevels;
		md3dDevice->CreateShaderResourceView(displacement2DList[i].Get(), &srvDesc, hDescriptor);

		// next descriptor
		hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	}

	mDisplacementMapHeapIndex = (UINT)tex2DList.size();
#endif // EX2
...
#ifdef EX2
	mSkyTexHeapIndex = (UINT)tex2DList.size() + (UINT)displacement2DList.size();
#else
...
#ifdef EX2
	skyRitem->DisplacementMapIndex = -1;
	boxRitem->DisplacementMapIndex = 0;
	globeRitem->DisplacementMapIndex = -1;
	gridRitem->DisplacementMapIndex = 1;
#endif // EX2
...
#ifdef EX2
		leftCylRitem->DisplacementMapIndex = 0;
		rightCylRitem->DisplacementMapIndex = 0;
		leftSphereRitem->DisplacementMapIndex = -1;
		rightSphereRitem->DisplacementMapIndex = -1;
#endif
