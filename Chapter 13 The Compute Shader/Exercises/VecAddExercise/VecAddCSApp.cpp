...
void VecAddCSApp::DoComputeWork()
{
	...
	mCommandList->SetComputeRootUnorderedAccessView(2, mOutputBuffer->GetGPUVirtualAddress()); // output
...
XMFLOAT3 GenerateRandomVectorInRange(float min, float max, float& mag) {
	static std::random_device rd;
	static std::mt19937 gen(rd());
	std::uniform_real_distribution<float> dist(-1.0f, 1.0f);
	std::uniform_real_distribution<float> mag_dist(min, max);
...
void VecAddCSApp::BuildBuffers() {
	...
	{
		float mag = 0.f;
		data[i].v = GenerateRandomVectorInRange(1.0f, 10.0f, mag);
		data[i].mag = mag;
	}
...
void VecAddCSApp::BuildBuffers() {
	...
	{
		float mag = 0.f;
		data[i] = GenerateRandomVectorInRange(1.0f, 10.0f, mag);
	}
