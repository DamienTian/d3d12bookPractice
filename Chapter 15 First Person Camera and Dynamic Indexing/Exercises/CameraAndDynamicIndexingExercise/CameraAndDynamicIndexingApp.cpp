...
#define EX2
#define EX3 // ref: https://zhuanlan.zhihu.com/p/338564571
...
private:
    ...
    void BuildShapeGeometryEX3();
    void BuildPSOs();
    void BuildFrameResources();
    void BuildMaterials();
    void BuildRenderItems();
    void BuildRenderItemsEX3();
...
void CameraAndDynamicIndexingApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	if (GetAsyncKeyState('Q') & 0x8000)
		mCamera.Roll(XMConvertToRadians(static_cast<float>(-50.0f * dt)));
...
#ifdef EX3
	auto redBricksTex = std::make_unique<Texture>();
	redBricksTex->Name = "redBricksTex";
	redBricksTex->Filename = L"../../../Textures/bricks3.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), redBricksTex->Filename.c_str(),
		redBricksTex->Resource, redBricksTex->UploadHeap));

	mTextures[redBricksTex->Name] = std::move(redBricksTex);
#endif // EX3
}
...
void CameraAndDynamicIndexingApp::BuildRootSignature()
{
	...
	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(rootParameterSize, slotRootParameter,
...
void CameraAndDynamicIndexingApp::BuildDescriptorHeaps()
{
...
#ifdef EX3
	auto redBricksTex = mTextures["redBricksTex"]->Resource;
#endif // EX3
	...
	srvDesc.Format = redBricksTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = redBricksTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(redBricksTex.Get(), &srvDesc, hDescriptor);
#endif // EX3
}
...
#ifdef EX3
		,{ "TEXCOORD", 1, DXGI_FORMAT_R8_UINT, 0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }
#endif //EX3
	};
}
...
void CameraAndDynamicIndexingApp::BuildShapeGeometryEX3()
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData box = geoGen.CreateBox(1.0f, 1.0f, 1.0f, 0);

	size_t verticesCount = box.Vertices.size() * 5;
	std::vector<Vertex> vertices(verticesCount);

	int vertexIndex = 0; // current vertex index
	for (int j = 0; j < 5; ++j) {
		for(size_t k = 0; k < box.Vertices.size(); ++k, ++vertexIndex)
		{
			XMVECTOR offset = { 3.0f, 0, 0, 0 };
			XMVECTOR position = XMLoadFloat3(&box.Vertices[k].Position);
			position += offset * (j - 2);
			XMStoreFloat3(&vertices[vertexIndex].Pos, position);

			vertices[vertexIndex].Normal = box.Vertices[k].Normal;
			vertices[vertexIndex].TexC = box.Vertices[k].TexC;
			vertices[vertexIndex].TexI = j;
		}
	}
	
	size_t indicesCount = box.GetIndices16().size() * 5;
	std::vector<std::uint16_t> indices(indicesCount);
	int index = 0; // current index's index
	for (int j = 0; j < 5; j++)
	{
		for (int i = 0; i < box.GetIndices16().size(); i++, index++)
		{
			indices[index] = box.GetIndices16()[i] + 24 * j; // 24 vertices for each box, so increment by 24
		}
	}
...
void CameraAndDynamicIndexingApp::BuildRenderItemsEX3()
{
	auto boxRitem = std::make_unique<RenderItem>();
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 1.0f, 0.0f));
	XMStoreFloat4x4(&boxRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
